...
type Shader struct {
	...
	// this is true. Populated by Device.NewShaderNative; combinedUniformLayout
	// is set up at construction time so packUniforms works without a
...
func (s *Shader) compile() {
	...
	if s.nativeMode {
		// Native WGSL path: source bytes go straight to the device with
		// no translation and no struct-name rewriting. The author already
		// wrote the matching @group/@binding layout in their .wgsl files.
