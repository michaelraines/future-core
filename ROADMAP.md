...
| Vulkan: ring-buffer staging for `Texture.Upload` / `UploadRegion` | Future | Today every upload calls `vkDeviceWaitIdle` before returning so the CPU can safely overwrite the shared `d.stagingMapped` scratch buffer. Per-upload this is a whole-queue stall — visible as frame hitches on AA-buffer resize, atlas page grow, or any per-glyph upload path. Proper fix: a per-upload region in a ring buffer with a per-region fence. Upload writes into its own region + stores fence handle + returns immediately; ring-buffer allocator blocks only when the cursor laps a region whose fence hasn't signaled yet. Tried a cmd-buffer-only deferral (no staging partitioning) in the shutdown-perf pass — corrupted text glyphs because back-to-back `UploadRegion` calls raced the CPU overwriting staging against the GPU still copying the previous glyph out of it. See `future-core/internal/backend/vulkan/texture_gpu.go` Upload/UploadRegion. |
